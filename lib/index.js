...
export default {
    ...
    createVibloMarkdownPreviewPane ({ uri, type, key, path, contents = '' }) {
        if (!type || !key) {
            const match = REGEX_PREVIEW_URI.exec(uri)
            if (match) {
                return new VibloMarkdownPreviewPane({ uri, type: match[1], key: match[2], path, contents })
            }
        }

        return new VibloMarkdownPreviewPane({ uri, type, key, path, contents })
    },
