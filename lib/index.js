...
const {ATOM_URI, POST_EDITOR_REGEX} = require('./config')
...
export default {
    ...
    addWorkspaceOpener () {
        ...
        atom.workspace.addOpener((uri) => { // eslint-disable-line no-undef
            const isPostEditor = POST_EDITOR_REGEX.exec(uri)
            if (isPostEditor) {
    ...
    addPostEditorOpener () {
        ...
        atom.workspace.observeTextEditors((editor) => {
            if (editor) {
                const match = POST_EDITOR_REGEX.exec(editor.getPath())
    ...
    registerCommands () {
        ...
        atom.commands.add('atom-workspace', {
            ...
            'viblo:save': async () => await openPublishForm(atom.workspace.getActiveTextEditor().id)
        })
    }
}
