...
export default class VibloPostEditor {
    ...
    async initForExistsPost () {
        ...
        const post = PostManager.normalizePost(response.post.data)
        if (post) {
    ...
    registerEvents () {
        // this.textEditor.onDidSave(() => atom.commands.dispatch(
        //    atom.views.getView(atom.workspace.getActivePane()), 'viblo:publish')
        // )
    }
}
